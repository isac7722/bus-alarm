...
검색창에 버스 번호를 입력하면 해당 노선의 정류장을 지도·목록에서 고를 수 있습니다. 가까운 정류장과 전체 정류장을 전환해 현재 위치 주변에서 바로 타거나 다른 장소의 정류장을 미리 등록합니다. 정류장·방향을 고르면 검색한 버스가 선택된 상태로 대기·즐겨찾기 저장을 진행합니다. [검색 흐름과 예외 처리](docs/design/bus-number-search-flow-proposal.md)를 참고하세요.
